...
class ServerRepositorySql implements ServerRepositoryInterface
{
    ...
    {
        try {
            $server_list = DB::select('select s.id ,s.name , s.set_icon , (select count(*) from belonger_in_server as bs where server_id = s.id) as num_of_people from servers as s');
    ...
    {
        try {
            $server_list = DB::select('select s.id as server_id , s.name as server_name , s.set_icon ,  (select min(c.id) from channels c where c.server_id = s.id) as channel_id from servers as s inner join belonger_in_server as bs on s.id = bs.server_id  where bs.user_id = ?', [auth()->id()]);
